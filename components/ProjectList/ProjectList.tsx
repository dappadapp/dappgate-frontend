...
const ProjectList: React.FC<Props> = ({ title, apps }) => {
  const openInNewTab = () => {
    window.open("/apps/dappgate", "_blank", "noopener,noreferrer");
  };

  const openInNewTabTracker = () => {
    window.open("https://tracker.dappgate.io", "_blank", "noopener,noreferrer");
  };
  return (
    <div
      style={{
    ...
    >
      ...
      <div className="flex w-full items-center">
        ...
        <button className="rounded border border-[white] text-[#fff] text-sm border-opacity-5 py-3 px-6" onClick={() => openInNewTab()}>
          View App
        </button>
      </div>
      ...
      <div className="flex w-full items-center">
        <div className="flex-[2] gap-3 justify-start flex items-center">
          <Image
            src={`https://pbs.twimg.com/profile_images/1707960655490281472/3mrVLogY_400x400.jpg`}
            alt="Tracker logo"
            width={70}
            height={70}
          ...
          />
          <div className="flex flex-col gap-1 text-[#858585]">
            <span className="text-base text-white">Tracker</span>
            <span className="text-xs">DappGate Tracker</span>
            <span className="text-xs break-words">
            Track wallet transactions and discover your eligibility for blockchain airdrops! 
            </span>
          </div>
        </div>
        <button className="rounded border border-[white] text-[#fff] text-sm border-opacity-5 py-3 px-6" onClick={() => openInNewTabTracker()}>
          View App
        </button>
      </div>
