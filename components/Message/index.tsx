...
import { useAccount, useContractRead, useContractReads, useBlockNumber } from "wagmi";
...
const Message: React.FC<Props> = ({
  ...
  const { data: receivedMessagesData, refetch: getMessages } = useContractReads({
    contracts: Array.from(Array(Number(receivedMessageCount) || 0).keys()).map((i) => ({
      address: sourceChain.messageContractAddress as `0x${string}`,
  ...
  const receivedMessages = receivedMessagesData?.map((message: any, i) => {
    ...
    return (
      <div
        className="flex flex-col gap-2 bg-white/10 border-white border-[1px] rounded-lg px-8 py-2"
        key={i}
      >
    ...
    >
      <div className="flex flex-row w-full justify-between items-center">
        <h1 className={"text-3xl font-semibold"}>
          {isReceivedMessageOpen ? "Received Messages" : "Send Message"}
        </h1>
        <div
          className="relative cursor-pointer w-[32px]"
          onClick={() => setIsReceivedMessageOpen((prev) => !prev)}
        >
          <CommentsSvg />
          {receivedMessageCount ? (
            <div className="absolute right-0 top-0 bg-red-600 p-1 px-1.5 text-xs rounded-full leading-[100%]">
      ...
      {isReceivedMessageOpen ? (
        <>
          <div className="flex flex-col gap-8 mt-8">{receivedMessages}</div>
        </>
      ) : (
        <>
          <div
          ...
          <div className="flex text-base xl:text-lg font-semibold mt-5">
            Step 1: Enter the wallet address of the user you want to send the message to.
          </div>
