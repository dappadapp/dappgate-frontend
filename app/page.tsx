  ...
  const onChangeSourceChain = async (selectedNetwork: Network) => {
    ...
    if (chain) {
      try {
        ...
        if (error.code === 4001) return toast("You need to confirm the Metamask request in order to switch network.")
        ...
        <div className={"container mx-auto h-full flex flex-col"}>
          ...
          >
            <Tab.Group onChange={setTabIndex} selectedIndex={tabIndex}>
              <Tab.List className="px-6 py-2 bg-slate-700/40 rounded-2xl">
                <Tab as={Fragment}>
                  {({ selected }) => (
                    /* Use the `selected` state to conditionally style the selected tab. */
                    <button
                      className={`px-6 py-2 rounded-2xl text-white ${selected ? 'bg-slate-700' : 'bg-transparent'}`}
                    >
                      NFT Bridge {"(ONFT)"}
                    </button>
                  )}
                ...
                <Tab as={Fragment}>
                  {({ selected }) => (
                    /* Use the `selected` state to conditionally style the selected tab. */
                    <button
                      className={`px-6 py-2 rounded-2xl ml-2 text-white ${selected ? 'bg-slate-700' : 'bg-transparent'}`}
                    >
                      Token Bridge {"(OFT)"}
                    </button>
                  )}
            ...
            </Tab.Group>
            {
              tabIndex !== 0 ?
                ...
                >
                  ...
                  <div
                    className={
                      "flex flex-col justify-center items-center gap-6 w-full mt-20 select-none"
                    }
                  >
                    <MintButton
                  ...
                  <div className={`w-full flex flex-col gap-4 mt-8 transition-all overflow-hidden ${layerZeroTxHashes.length !== 0 ? "max-h-[1000px]" : "max-h-0"}`}>
                    <h1 className={"text-3xl font-semibold"}>Layer Zero Transactions</h1>
                    {layerZeroTxHashes.map((hash, i) => {
                      return (
                        <div key={hash} className="ml-4">Transaction #{i + 1}:
                          <a href={`https://layerzeroscan.com/tx/${hash}`} target="_blank" className="text-orange-400">
          ...
          <div className={"mt-auto pb-16 flex justify-between items-center"}>
            ...
            <div className={"flex gap-4 text-xl text-gray-400 "}>
              <a href="https://twitter.com/Dappadofficial" target="_blank" className={"hover:text-gray-100 transition-all"}>
                <FontAwesomeIcon icon={faTwitter} />
              </a>
              <a href={"https://discord.gg/dappadlaunchpad"} target="_blank" className={"hover:text-gray-100 transition-all"}>
