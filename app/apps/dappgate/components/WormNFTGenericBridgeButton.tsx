...
const WormNFTGenericBridgeButton: React.FC<Props> = ({
  ...
  const {
    ...
    args: [
      ...
      `${"0x000000000000000000000000"}${account?.toString().replace(
        "0x",
        ""
      ) + ""}`,
      block
    ...
    if (!sendFrom) {
      ...
      if (
        error?.message.includes(
          "Worm: destination chain is not a trusted source"
        )
      ) {
        return toast(
      ...
      return toast(
        `Make sure you have more and you're on the correct network.`, {autoClose: 6000}
      );
    }
    if (!isSuccess) {
  ...
  const onApprove = async () => {
    ...
    if (!approve) {
      ...
      if (
        error?.message.includes(
          "Wormhole: destination chain is not a trusted source"
        )
      ) {
        return toast(
          "It looks like the bridge between these chains are closed."
