...
const ONFTGenericBridgeButton: React.FC<Props> = ({
      ...
      setEstimatedGas(
        `${(
          Number(
            ...
            coefficient +
          Number(((bridgeFeeData as bigint) * BigInt(coefficient)) / BigInt(1e18)) /
            coefficient
  ...
  const onBridge = async () => {
    ...
    if (!sendFrom) {
      ...
      return toast(
        `Make sure you have more than ${Number(
          ethers.formatEther(
            (
              BigInt(((gasEstimateData as any)?.[0] as string) || "0") +
              BigInt((bridgeFeeData as string) || "0") +
        ...
        )?.toFixed(4)} ${sourceChain.symbol} and you're on the correct network.`,
        { autoClose: 6000 }
      );
    }
    if (!isSuccess) {
